...
const (
	addSitesSql  = `INSERT INTO sites(author, dir, mapping, created, updated) VALUES(?, ?, ?, ?, ?)`
	checkSiteSql = `SELECT COUNT(*) FROM sites WHERE dir = ? or mapping = ?`
	getAllSql    = `SELECT id, author, dir, mapping, created, updated FROM sites`
)
...
func (ls *LiteStore) Store(author, dir, path string) error {
	now := time.Now().UnixNano() / int64(time.Second)
	_, err := ls.LiteDB.Exec(addSitesSql, author, dir, path, now, now)
	return err
}
...
func (ls *LiteStore) GetAll() []Site {
	...
	for rows.Next() {
		var site Site
		err := rows.Scan(&site.Id, &site.Author, &site.Dir, &site.Mapping, &site.Created, &site.Updated)
		if err != nil {
			continue
		}
